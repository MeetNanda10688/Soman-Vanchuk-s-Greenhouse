...
function setup() 
{
  ...
  raysGroup = createGroup(); //raysGroup = new Group();
...
function makeRay()
{
   ...
   if (frameCount % 60 === 0) 
   {
    ...
    raysGroup.setLifetimeEach(134); //to avoud memory leak
     
     //Add overlap functions and callback functions
...
function charge1(ray){
  ray.remove();
  absorbed1 += 1;
}

function charge2(ray){
  ray.remove();
  absorbed2 += 1;
}

function temp_rise (ray){
  ray.remove();
  temp += 1;
}
